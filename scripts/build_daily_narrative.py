...
def build(data: dict) -> str:
    ...
    if data.get("crawl_alerts"):
        ...
        # 定性一行(2026-07-29 白杨化重构):红字是真实的,但别误读成主线在恶化——
        # 自有站百度线已定性为背景任务(发现链卡「关联主体」,钥匙在 Jack);
        # 获客主线 = 站外容器(百家号自动发文)+ AI 引用(月检),另有报告。
        L.append("_↳ 百度自有站线=背景任务(卡关联主体,钥匙在Jack);主线=站外容器+GEO,见分发报告与月度AI引用检_")
        L.append("")
