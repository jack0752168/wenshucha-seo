...
def run(label,params,maxpg=30):
    ...
    for c in cases:
        ...
        by[y][1]+=1
    print(f"\n===== {label} =====")
    print("  total:",call(dict(params,size=1)).get("total"),"| 取样:",len(cases),"| 案由纯度:",causes.most_common(3))
    for y in sorted(by):
        a,n=by[y]
        if n>=5: print(f"    {y} | {a}/{n} | {a/n*100:.1f}%")
run("B链 交通官司(机动车交通事故责任)", {"q":"交通事故","cause":"机动车交通事故责任","caseType":"民事","lastYears":"3","excludeAnnouncement":"1"})
run("C链 劳动争议(对照)", {"q":"违法解除","cause":"劳动争议","caseType":"民事","lastYears":"3","excludeAnnouncement":"1"})
