...
### ★ 第二条业务线（2026-09-15 Jack 定案）：裁判文书数据定制报告 → 读者是「写论文的人」

**卖的是服务**：按研究主题在 1.6 亿全库上出统计报告（逐年案件量／案由构成／地域审级画像／关键词交叉／
法院观点归类／裁判结果分布），交付 Word+Excel+PPT+图表，每个数字附检索式。落点 `https://www.wenshucha.com/data-report/`。
**读者是「要在论文里放实证数据的人」**：法学硕博生、青年教师、写期刊论文的律师、做课题的学者、智库/媒体研究员。
**他们的痛点不是「办案」，是「审稿人会问数据哪来的、怎么抽的、能不能复现」。**

- **内容配比**：本线在百家号／知乎的日配额里固定占 **3/10**（Jack 说「全部业务线都往那一块偏」，
  但办案线是存量主力，不整体让位）。
- **口吻**：方法论＋数据示范，不是广告。每篇要做一件事——**当场跑一个真实统计给读者看**
  （案由×年份／省份分布／审级占比／关键词共现），把口径、检索式、局限写清楚，然后一句「这类统计我们做成了服务」。
- **CTA 统一**：短链 `www.wenshucha.com/paper`（百家号）／`/paper-zh`（知乎）／`/paper-wx`（公众号），
  302 到 `/data-report/` 带 utm，百度统计与首方像素都能归因。**每篇最多一处，放文末。**
- **诚实铁律（比办案线更严，因为读者是审稿人）**：
  ① 不写「胜诉率」，只写裁判结果分布并注明口径（[[feedback_seo_no_vanity_metrics]]）；
  ② 任何案件量必须同时给记录数与相异案号数，并提 2023 年后公开量收缩；
  ③ 抽样必须写种子与 n（reference_es_sample_needs_random_score）；
  ④ 案由是 tokenized OR 不是精确标签，报纯度（reference_wsc_cause_filter_not_exact）。
- **这条线在百度好不好打（2026-09-15 真搜 14 词实测）**：3 干净 / 10 毙 / 1 无量，毙率 71%，比办案线好一档但**不是净土**。
  首页压着的是**高校研究中心/图书馆页**（11/14，不答题但占位）、**法院官网**（7/14，带「案件量/统计/案由/省份」词根就招）、
  **法行宝**（3/14，带「怎么查/导出/数量」词根）、**星谱自家**（4/14，9/11 那篇实证数据源文已在卡位）。
  ⇒ 选词往「方法/写作规范/审稿追问/可视化/口径设计」走，避开裸「统计/案件量/怎么查」；台账「📚」节表头有本线专用判据。
- **选题池**：`keyword-ledger.md` 新增「📚 学术/论文线」一节，只从那里取词；真搜验词规则不变。
...
### 🟢 2026-09-07 实测推翻上面整段「当日公开天花板 5 篇」——那是 Lv5 时代的一次性观测

09-07 17:2x 全量拉 `/api/v4/creators/creations/v2/article`（翻到 offset 80，共 83 条）
按 `created_time` 归日聚合，**天花板不存在**：
...
| 08-28 | 10 |
| 09-03 | 7 |
| 09-07 | 6（含本轮这篇，发前已有 5） |

⇒ 「已发布列表今日条数 ≥5 就停手」这条**必须废掉**。它会让任务在配额充裕时天天少发，
是纯净损失。同日记忆 [[reference_zhihu_capacity_three_channels]] 的口径一致：
**专栏区未观测到滚动上限，回答区才有 ~40/7d 的硬闸门。**
判「今天还能不能发专栏」不看条数，看内容供给（能不能拿到干净数据链）。

### 🚨 2026-09-07：「发布中...」按钮不动 **≠** 没发出去，别据此判失败

本轮点发布后，按钮恒为「发布中...」、底部状态恒为「N 分钟前 · 草稿」，
持续 38 秒无变化，形状与 08-18 那堵墙一模一样。**但文章其实已经发布成功了。**

**正确判据（按可信度排序）：**
1. **重载 `/edit` 看按钮文案** —— 变「更新」＝已发布；仍是「发布」＝真没发出去。
2. **creator API 首位是不是本篇** —— `/api/v4/creators/creations/v2/article?limit=5`，
   看 `data[0].data.id` 与 `created_time`。
3. 线上 `/p/{id}` 的 `.Post-RichTextContainer` 段数。

❌ **不可信**：发布按钮文案（会永久停在「发布中...」）、底部「· 草稿」状态行
（重载前不刷新）。08-18 那次「点了发布仍回落成草稿」的结论**至少在按钮文案这一项上是误判**，
它当时没做「重载后看按钮是否变更新」这一步。
...
```
**🔴 2026-09-08 实测：上面第 5-8 步的封面链路已被平台改版，照旧写法会卡死。新链路如下：**
```
5'. 点封面框（JS 五连派发 mousedown/mouseup/click 到「选择封面」那个 div 的父容器即可，
    不必算坐标、不必点"图片图标上移 32px"）→ 弹框**直接就是全屏 980×671 展开态，没有收起态**，
    「等 6 秒展开后重新取坐标」这一步可以删。
6'. 标签名「AI封图」**已改名「AI封面」**（判据用 .cheetah-modal-content 的 innerText）。
7'. 面板改版成四件套：热门模板 / 封面描述 / 从正文总结 / 封面文字 / 生成。
    **旧手册的「根据全文智能生成封面」这个链接已不存在。**
    正确顺序：先点「从正文总结」→ 等约 7 秒，描述与封面文字自动填好 → 再点「生成」→ 等 25-35 秒。
8'. 生成结果**只有 2 张，不是 3-4 张**，且**右边那张常是模板占位文字**
    （2026-09-08 实测右图渲染成「装饰不遮档文字」）。⇒ **选之前必须截图看文案**，
    选中后标签会变「AI封面(1)」、右侧预览同步、按钮变「确定 (1)」，再点确定。
```
...
## 🔴 2026-09-01 三条新实测（收官轮踩到，都改判据）

**① 百家号封面弹框已改版，旧 DOM 判据会误报「弹框没开」。**
标签名 `AI封图` → **`AI封面`**；入口从文字链「根据全文智能生成封面」→ **按钮「AI一键生成封面」**，
且**新版打开即为展开态**，不再需要「等 6 秒展开后再点」那一步。
本轮用旧关键字 `AI封图` 查 DOM 判定「弹框未开」，实际截图显示弹框早就开了——**差点重点一次把它关掉**。
→ 判据改为查 `AI封面` / `免费正版图库` 任一；生成耗时仍约 20-40 秒（本轮 40 秒才出图），
   出 **2 张**候选（不是 3-4 张），默认**不选中**，必须先点图再点「确定 (1)」。
→ 选了 AI 封面后，「创作声明 · 采用AI生成内容」会被平台自动勾上，属正常，别去取消。

**② 知乎 paste 后等 38 秒仍会空落库 —— 时间要拉到 60 秒以上，且必须重载复核。**
08-18 记的是「≥30 秒」，本轮按 38 秒走，重载 /edit 后**服务端只剩 1 个空 block，标题存住了正文全丢**。
在 /edit 页原地重灌、等满 60-70 秒见底部「刚刚 · 草稿」+ 保存勾，再重载才拿到完整 23/30 blocks。
→ **改为：paste 后等 ≥60 秒，且必须看到底部状态变成「刚刚 · 草稿」带勾，再重载 /edit 复核。**
→ **早期告警信号**：注入后若弹出「草稿加载中，请等待加载完成后再次修改」模态框
   （/write 拿到草稿 id 自动跳 /edit 的竞态），**它出现就意味着这次注入多半没落库**，直接准备重灌。
→ 再次坐实：`blocks` 读回 30/30、底部字数 1537 全对，**都不能当落库判据**，只有重载 /edit 算数。

**③ 自我蚕食预检必须从 `state/bjh_metrics.jsonl` 抽标题，不要从 PUBLISH-LOG 正则抽。**
本轮用 PUBLISH-LOG 正则只抽到 **71/150** 条标题，导致连撞自己 3 次才发现：
「开庭前怎么预判法官会问什么」撞 08-21、「输了的案子怎么复盘」撞 08-14、「律师函怎么写才有用」撞 08-13
（三个词的百度首页第 1-4 位都是我们自己的文章）。metrics.jsonl 一行一篇带 `title` 字段，才是全量。
```bash
python3 -c "import json;[print(json.loads(l)['title']) for l in open('state/bjh_metrics.jsonl') if l.strip()]"
```
**附带结论：「程序概念型」词（执行异议之诉／诉讼请求／财产保全／诉讼费／送达）是法院官网主场，整类别再试**
——本轮 4 个此类词全部被法院/检察院官网 4-6 条压顶。能过的一律是「检索／数据／工具」向的词。

### 🔴 2026-09-02 09:3x 修正上面那条「知乎专栏当日公开约 2 篇硬墙」——**不能按篇数用**

本轮发出**今日第 3 篇专栏**（`p/2078416236736673575`），**一次点发布即公开**：
URL 从 `/edit` 跳 `/p/{id}`、线上 `.Post-RichTextContainer` **43 段与源稿全等**、
无「发布中…」卡死、无回落「刚刚 · 草稿」。前两篇分别在 08:0x（`p/2078387180733461373`）
和 08:4x（`p/2078400984892584880`），创作中心列表显示「发布于 1 小时前 / 56 分钟前」，均已公开。

⇒ 08-18 那次「提交 9 篇只有最早 2 篇公开」的真实变量**大概率是提交密度，不是当日绝对篇数**——
当时 9 篇全挤在 11:40 之后；本轮 3 篇分散在 08:0x / 08:4x / 09:3x，每轮 1 篇、间隔 1 小时以上，
正好是 SKILL 要求的拟人化分散投放节奏。

**How to apply：**
1. **不要再因为「今天已发 2 篇专栏」就跳过知乎**——那会白白丢掉产能，且这个自我设限没有当前证据支持。
2. 但**不许反向外推**：本轮只证到第 3 篇，没证第 4、5 篇。每篇仍逐一走三重判据
   （发布前 `/api/articles/<id>/draft` 验服务端 `<p>` 数 → URL 是否跳 `/p/{id}` → 线上 `.Post-RichTextContainer` 段数）。
3. 一旦再次出现「按钮变发布中不动 + 底部恒为『刚刚 · 草稿』」，仍按旧结论处置：**不重试、次日开 /edit 重发**。

---

## 🔧 2026-09-03 07:xx 三条链路更正（都省回合，建议作为标准手法）

### ① 百家号封面 UI 再改版：「AI一键生成封面」按钮**在 DOM 里找不到**，走下方面板才是活路

09-01 那条「入口＝按钮『AI一键生成封面』」本轮**只对了一半**——按钮在截图上清清楚楚，
但 `[...document.querySelectorAll('*')].filter(e=>e.textContent.includes('一键生成封面'))` **命中 0**，
`document.body.innerText` 里也搜不到这七个字（疑似 shadow DOM，`shadowRoot` 计数 1）。
照旧法取元素必抛 `Cannot read properties of undefined (reading 'click')`。

**破法＝绕开那个按钮，走它下方那块面板：**
```js
// 1) 点「从正文总结」——会自动把「封面描述」和「封面文字」两个输入框都填好
//    本轮自动填出：描述=极简构图与大面积留白…；文字=「AI质疑律师怎么接」
// 2) 再点「生成」
```
这两个词都在**主文档**里，`innerText` 精确等值匹配拿得到，稳。
生成耗时本轮 **约 24 秒**（比 09-01 记的 40 秒快），出 **2 张**候选、**默认不选中**，
点图后按钮变「确定 (1)」再点确定。选完「创作声明·采用AI生成内容」被自动勾上，属正常别取消。

### ② 本页所有交互一律 `element.click()`，坐标点击**全部静默失效**

本轮三处实测：「发文设置」「选择封面框」「AI封面 tab」用 `computer left_click`
（坐标已按 `1568/window.innerWidth=1.037` 正确换算）**一次都没生效，且不报错**。
换成 `element.click()` 三处全部一次通过。复证 [[reference_submit_buttons_need_element_click]]，
**今后百家号编辑页直接跳过坐标点击这条路，省一半回合。**
⚠️ 但「发文设置」有 **2 个**同名元素，`filter` 后一次点两个＝开了又关，必须只点 `t[t.length-1]`。

### ③ 标题注入不必用真实键盘，`execCommand('insertText')` 一次到位且不吃字符
...
```
本轮 23 字标题**全等落地，没有触发 [[reference_bjh_title_type_eats_first_chars]] 的吃首字符**。
比 `computer type` 少一个回合，也不用事后补字符。正文仍走 `UE_V2.instants.ueditorInstant0.setContent`
（本轮 `getContentTxt().length` 2177 与源文全等，计数器当场 0→2177，`<p>` 31 含 1 个自补空段）。

---

### 🔴 2026-09-08 · 百家号「点发布毫无反应」的真根因：正文首段必须是标签，不能是裸文本节点

**症状**：JS 注入标题+正文全部成功（逐段 diff=0、字数计数器正常显示 1801、封面挂好），
点「发布」后**页面不跳转、不弹任何提示、零发布 API 请求**。连点 5 次都一样，
`computer` 真实点击和 `dispatchEvent` 五连派发都无效，表单也扫不出任何校验错误。

**唯一的线索在打点里**：`urlkey=文章长度打点-level1-0` —— 平台认为文章长度是 0。
（注意这跟 08-21 那条「底栏字数恒为 0」不是一回事，本次字数显示是正常的 1801。）

**真根因要读 console 才看得到**：
```
Error: Syntax error, unrecognized expression: 检索报告交出去三个月，……<div data-diagnose-id="…">…</div>
  at fa.tokenize (jquery@2.2.3)  at ex (bjh-mfe/js/news.*.js)
```
百家号发布前的处理函数 `ex()` 把**整段正文 HTML 字符串塞进 jQuery `$()`**。
jQuery 2.x 只在字符串**以 `<` 开头**时才走 HTML 解析，否则当选择器解析。
而 `d.execCommand('insertText')` 逐段注入的产物是：**首段是裸文本节点，其余才是 `<div>`**，
于是 innerHTML 以中文开头 ⇒ 被当选择器 ⇒ 抛异常 ⇒ 整条发布链路静默中断。

**破法（注入完正文后必做）**：
```js
const esc=s=>s.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
d.body.innerHTML = P.map(p=>'<div>'+esc(p)+'</div>').join('');   // 保证以标签开头
window.UE_V2.instants.ueditorInstant0.fireEvent('contentchange'); // 08-21 那条，仍要
// 再补一次真实键盘事件（点正文 → 敲一字 → Backspace），然后点发布，一次即成
```
**自检判据**：点发布前先跑 `d.body.innerHTML.trim().startsWith('<')`，false 就一定发不出去。
**别再把「点了没反应」归因成按钮要 element click 或要点两次——那两条在本例里都无效。**

### 🔴 2026-09-08 · JS rect 坐标不能直接喂 computer 点击（本机差 1.037 倍）

知乎发布按钮 `getBoundingClientRect` 中心 = (1125, 684)，**连点三次全部落空**，
只触发了草稿保存，服务端 state 一直是 draft。实际 computer 坐标系里按钮在 **(1166, 709)**。
比值 = 1568/1512 = **1.037**（click frame 宽 / `window.innerWidth`），即 [[reference_chrome_mcp_rect_vs_click_frame]]。
**规则：凡拿 JS rect 去喂 `computer.left_click`，一律乘 `frameWidth/innerWidth`；
frameWidth 从每张截图的 "coordinate frame" 那行读，别照抄旧值。**
（本轮百家号那侧没踩到，是因为百家号那些点击我走的是 `element.click()`，不经坐标系。）

### 🔴 2026-09-08 · 知乎发布成败只认 draft 接口，别信 v4 文章接口

- `/api/v4/articles/<id>` 对自己刚发的文章**恒返「请求参数异常，请升级客户端后重试」**，
  这**不是**发布失败的证据。
- 唯一可信判据：`/api/articles/<id>/draft` 的 `state` 字段，`draft` → `published`。
- 按钮显示「发布中...」不动同样不代表失败（沿用 09-07 那条），以 draft 接口读数为准。

### 🚦 2026-09-08 实测：知乎回答滚动 7 天窗口已 40/40 顶死

本轮查 `/api/v4/members/<me>/answers` 实算：**近 7 天 40 条、近 24 小时 7 条**，
正好撞上 [[reference_zhihu_rolling_week_window_40]] 那条硬闸门。
**⇒ 当天再投回答必被限流，5 篇 banked 积压只能等窗口滑出，不是稿子的问题。**
清积压前**必须先算这个 7 天数**，别只看「今天发了几篇」。
